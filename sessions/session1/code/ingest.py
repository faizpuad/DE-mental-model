    ...
    def validate_ingestion(self, expected_rows):
        try:
            ...
            if actual_rows < expected_rows * 0.95:
                logger.warning(f'Significant data loss: {(expected_rows - actual_rows)} rows missing')
            else:
    ...
    def get_data_summary(self):
        try:
            ...
            summary_queries = [
                ...
                ('Total invoices', 'SELECT COUNT(DISTINCT invoiceno) FROM bronze.raw_transactions'),
                ('Total products', 'SELECT COUNT(DISTINCT stockcode) FROM bronze.raw_transactions'),
                ('Total customers', 'SELECT COUNT(DISTINCT customerid) FROM bronze.raw_transactions WHERE customerid IS NOT NULL'),
                ('Total countries', 'SELECT COUNT(DISTINCT country) FROM bronze.raw_transactions'),
                ('Date range start', 'SELECT MIN(invoicedate) FROM bronze.raw_transactions'),
                ('Date range end', 'SELECT MAX(invoicedate) FROM bronze.raw_transactions'),
                ('Total revenue', 'SELECT SUM(quantity * unitprice) FROM bronze.raw_transactions WHERE quantity > 0')
            ]
            ...
            for name, query in summary_queries:
                ...
                if name == 'Total revenue':
                    logger.info(f'  {name}: £{result:,.2f}' if result else f'  {name}: N/A')
                else:
