...
CREATE TABLE gold.fact_sales_daily_enhanced (
    ...
    CONSTRAINT uq_fact_sales_daily_enhanced_date UNIQUE (date_id),
    CONSTRAINT fk_fact_sales_daily_enhanced_date
        FOREIGN KEY (date_id) REFERENCES silver.dim_date(date_id)
);

CREATE INDEX idx_fsde_date ON gold.fact_sales_daily_enhanced (date DESC);
CREATE INDEX idx_fsde_year_month ON gold.fact_sales_daily_enhanced (year, month);

COMMENT ON TABLE gold.fact_sales_daily_enhanced IS 'Enhanced daily sales metrics with trend analysis';
